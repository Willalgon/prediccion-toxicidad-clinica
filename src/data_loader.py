...
class ClinToxDataset(Dataset):
    def __init__(self, dataframe, max_bits=2048):
        self.data = dataframe
    ...
    def smiles_to_fingerprint(self, smiles_str):
        ...
        if mol is None:
            return np.zeros(self.max_bits, dtype=np.float32)
        # Morgan Fingerprint
        fp = rdMolDescriptors.GetMorganFingerprintAsBitVect(
            mol, radius=2, nBits=self.max_bits
        )
...
def get_dataloaders(csv_path, batch_size=32):
    ...
    train_val_df, test_df = train_test_split(
        df, test_size=0.15, random_state=42, stratify=df["CT_TOX"]
    )
    # Obtenemos conjunto de validación
    train_df, val_df = train_test_split(
        train_val_df, test_size=0.15, random_state=42, stratify=train_val_df["CT_TOX"]
    )
